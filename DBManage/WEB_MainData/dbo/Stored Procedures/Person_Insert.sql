...
CREATE procedure [dbo].[Person_Insert]	-- not null ->	+
(	 @i_RowStatus			nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_Person_ID			uniqueidentifier	= newid()
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@TranCount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_Person_Insert
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select @CntParam = N'Person_Code="'+try_cast(@u_Person_Code as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Person_Insert.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------		
	...
	if (@e = 0) begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'Person_Insert.Insert', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_Person_Insert	-- rollback if transaction was created outside and this procedure creates only save point
	end
	-- Output parameters of the procedure 
	
	if @e > 0 select @o_IsError = 1, @o_Entity_ID = null else select @o_IsError = 0, @o_Entity_ID = try_cast(@u_Person_ID as nvarchar(36))
