...
CREATE procedure [dbo].[EntityType_Update]	-- not null ->	+
(	 @i_EntityType_ID	nvarchar(250)	-- sys param	+
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_EntityType_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		if (@i_EntityType_Code is null ) select Id=50101, Msg=formatmessage(50101, 'EntityType_Code' ) else if (@u_EntityType_Code	is null ) select Id=50103, Msg=formatmessage(50103, 'EntityType_Code' ,try_cast(@i_EntityType_Code as nvarchar(250)));set @e=@e+@@rowcount
		--== place your params here =
		...
		select @CntParam = N'EntityType_ID="'+try_cast(@u_EntityType_ID as nvarchar(max))+N'"', @Cnt=-1
		...
		select @CntParam = N'EntityType_ID="'+try_cast(@u_EntityType_ID as nvarchar(max))+N'",'+N'EntityType_Code="'+try_cast(@u_EntityType_Code as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_EntityType', @CntParam, @Cnt out; if (@Cnt>0) select Id=50123, Msg=formatmessage(50123, try_cast(@u_EntityType_Code as nvarchar(250)), 'EntityType'); set @e=@e+@@rowcount
		--== place your params here =

	end try	begin catch	
		select Id=50100, Msg=formatmessage(50100, N'EntityType_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					from dbo.p_EntityType
					where EntityType_ID = @u_EntityType_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
				(select 
					 ...
					 ,EntityType_Codee  = @u_EntityType_Code
					 --== place your params here =
	...
	begin
		-- EntityType
		if (@e = 0) begin try
			update dbo.p_EntityType set
				 RowStatus	  = @u_RowStatus	 
				,Update_DTS	  = @u_Update_DTS	 
				,Modified_User_ID  = @u_Modified_User_ID
				,EntityType_Code	= @u_EntityType_Code
				--== place your params here = 

			where EntityType_ID = @u_EntityType_ID
		end try begin catch 
			select Id=50100, Msg=formatmessage(50100, N'EntityType_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_EntityType_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
