...
create procedure dbo.ProcessPhase_Delete	-- not null ->	+
(	 @i_ProcessPhase_ID		nvarchar(250)		-- usr param	+
	...
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_ProcessPhase_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'ProcessPhase.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	-- ProcessPhase
	if (@e = 0) begin try
		update dbo.p_ProcessPhase set RowStatus = @RowStatus, Update_DTS = @u_Update_DTS,Modified_User_ID  = @u_Modified_User_ID where ProcessPhase_ID = @u_ProcessPhase_ID
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'ProcessPhase.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_ProcessPhase_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
