...
CREATE procedure [dbo].[Contragent_Delete]	-- not null ->	+
(	 @i_AppUser_ID									nvarchar(250)		-- sys param	+
	...
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_Contragent_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'Contragent.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	-- Contragent
	if (@e = 0) begin try
		update dbo.p_Contragent set RowStatus = @RowStatus, Update_DTS = @u_Update_DTS,Modified_User_ID  = @u_Modified_User_ID where Contragent_ID = @u_Contragent_ID
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'Contragent.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_Contragent_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
