...
create procedure dbo.Line_to_TitleObject_Update	-- not null ->	+
(	 @i_Line_to_TitleObject_ID		nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_TitleObject_ID				uniqueidentifier	= try_cast(ltrim(rtrim(@i_TitleObject_ID		))	as nvarchar(255))
		,@u_Line_to_TitleObject_ID		uniqueidentifier	= try_cast(ltrim(rtrim(@i_Line_to_TitleObject_ID))	as nvarchar(255))
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@TranCount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_Line_to_TitleObject_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select @CntParam = N'Line_to_TitleObject_ID="'+try_cast(@u_Line_to_TitleObject_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_Line_to_TitleObject', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'Line_to_TitleObject',try_cast(@u_Line_to_TitleObject_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		
		select @CntParam = N'Line_to_TitleObject_ID="'+try_cast(@u_Line_to_TitleObject_ID as nvarchar(max))+N'",'+N'Line_ID="'+try_cast(@u_Line_ID as nvarchar(max))+N'",'+N'TitleObject_ID="'+try_cast(@u_TitleObject_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_Line_to_TitleObject', @CntParam, @Cnt out; if (@Cnt>0) select Id=50123, Msg=formatmessage(50123, try_cast(@u_Line_to_TitleObject_ID as nvarchar(36)), 'Line_to_TitleObject'); set @e=@e+@@rowcount

		--== place your params here =

	end try	begin catch	
		select Id=50100, Msg=formatmessage(50100, N'Line_to_TitleObject_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					,Line_ID
					,TitleObject_ID	
					from p_Line_to_TitleObject
					where Line_to_TitleObject_ID = @u_Line_to_TitleObject_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
	...
	begin
		-- Line
		if (@e = 0) begin try
			update dbo.p_Line_to_TitleObject set
				 RowStatus	  = @u_RowStatus	 
				,Update_DTS	  = @u_Update_DTS	 
				,Modified_User_ID  = @u_Modified_User_ID
				,Line_ID  = @u_Line_ID 
				,TitleObject_ID	  = @u_TitleObject_ID	 
			where Line_to_TitleObject_ID = @u_Line_to_TitleObject_ID
		end try begin catch 
			select Id=50100, Msg=formatmessage(50100, N'Line_to_TitleObject_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_Line_to_TitleObject_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
