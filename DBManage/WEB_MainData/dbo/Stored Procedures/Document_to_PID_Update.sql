...
create procedure dbo.Document_to_PID_Update	-- not null ->	+
(	 @i_Document_to_PID_ID	nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_Document_to_PID_ID		uniqueidentifier	= try_cast(ltrim(rtrim(@i_Document_to_PID_ID	))	as nvarchar(255))
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@TranCount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_Document_to_PID_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select @CntParam = N'Document_to_PID_ID="'+try_cast(@u_Document_to_PID_ID as nvarchar(max))+N'",'+N'Document_ID="'+try_cast(@u_Document_ID as nvarchar(max))+N'",'+N'PID_ID="'+try_cast(@u_PID_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_Document_to_PID', @CntParam, @Cnt out; if (@Cnt>0) select Id=50123, Msg=formatmessage(50123, try_cast(@u_Document_to_PID_ID as nvarchar(36)), 'Document_to_PID'); set @e=@e+@@rowcount

		--== place your params here =

	end try	begin catch	
		select Id=50100, Msg=formatmessage(50100, N'Document_to_PID_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					,Document_ID
					,PID_ID	
					from p_Document_to_PID
					where Document_to_PID_ID = @u_Document_to_PID_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
	...
	begin
		-- Document
		if (@e = 0) begin try
			update dbo.p_Document_to_PID set
				 RowStatus	  = @u_RowStatus	 
			...
			select Id=50100, Msg=formatmessage(50100, N'Document_to_PID_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_Document_to_PID_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
