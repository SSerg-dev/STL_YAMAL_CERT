...
create procedure dbo.Employee_to_GeneralDocument_Delete	-- not null ->	+
(	 @i_AppUser_ID									nvarchar(250)		-- sys param	+
	,@i_Employee_to_GeneralDocument_ID	nvarchar(250)		-- usr param	+
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Employee_to_Document_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		-- Not null check and Convert check	
		if (@i_Employee_to_GeneralDocument_ID	is null ) select Id=50101, Msg=formatmessage(50101, 'Employee_to_GeneralDocument_ID'	) else if (@u_Employee_to_GeneralDocument_ID	is null ) select Id=50108, Msg=formatmessage(50108, 'Employee_to_GeneralDocument_ID'	,try_cast(@i_Employee_to_GeneralDocument_ID	as nvarchar(250)));set @e=@e+@@rowcount
		...
		select Id=50100, Msg=formatmessage(50100, N'Employee_to_GeneralDocument.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	...
	if (@e = 0) begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'Employee_to_GeneralDocument.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Employee_to_Document_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
