...
-- =============================================
create procedure Contragent_Update		-- not null ->	+
(	 @i_Contragent_ID	nvarchar(250)	-- sys param	+
	...
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_Contragent_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		if (@i_Contragent_ID	is null ) select Id=50101, Msg=formatmessage(50101, 'Contragent_ID'		) else if (@u_Contragent_ID		is null )	select Id=50108, Msg=formatmessage(50108, 'Contragent_ID'	,try_cast(@i_Contragent_ID		as nvarchar(36)))	;set @e=@e+@@rowcount
		...
		exec Get_RowCount 'p_Contragent', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'Contragent',try_cast(@u_Contragent_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		
		select @CntParam = N'Contragent_ID="'+try_cast(@u_Contragent_ID as nvarchar(max))+N'",'+N'Contragent_Code="'+try_cast(@u_Contragent_Code as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Contragent_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					where Contragent_ID = @u_Contragent_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
				(select 
					 ...
					 ,Contragent_Codee  = @u_Contragent_Code
	...
	begin
		-- Contragent
		if (@e = 0) begin try
			update dbo.p_Contragent set
				 RowStatus	  = @u_RowStatus	 
				,Update_DTS	  = @u_Update_DTS	 
				,Modified_User_ID  = @u_Modified_User_ID
				,Contragent_Code	= @u_Contragent_Code
				,Description_Eng	= @u_Description_Eng
				,Description_Rus	= @u_Description_Rus
			where Contragent_ID = @u_Contragent_ID
		end try begin catch 
			select Id=50100, Msg=formatmessage(50100, N'Contragent_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_Contragent_Update-- rollback if transaction was created outside and this procedure creates only save point
	end
