...
create procedure [dbo].[Employee_to_Document_Delete]	-- not null ->	+
(	 @i_AppUser_ID									nvarchar(250)		-- sys param	+
	,@i_Employee_to_Document_ID	nvarchar(250)		-- usr param	+
	...
	begin try
		...
		if (@i_Employee_to_Document_ID	is null ) select Id=50101, Msg=formatmessage(50101, 'Employee_to_Document_ID'	) else if (@u_Employee_to_Document_ID	is null ) select Id=50108, Msg=formatmessage(50108, 'Employee_to_Document_ID'	,try_cast(@i_Employee_to_Document_ID	as nvarchar(250)));set @e=@e+@@rowcount
		...
		select Id=50100, Msg=formatmessage(50100, N'Employee_to_Document.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	...
	if (@e = 0) begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'Employee_to_Document.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
