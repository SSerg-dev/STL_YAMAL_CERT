...
-- Evaluated and refactored by RAlizade (2018-09-10)
-- Evaluated and refactored by Asmirnov (2018-09-13)
-- =============================================

CREATE procedure [dbo].[Register_To_Document_Delete]	-- not null ->	+
(	 @i_AppUser_ID									nvarchar(250)		-- sys param	+
	,@i_Register_to_Document_ID						nvarchar(250)		-- usr param	+
	...
	declare 
		...
		,@u_Register_to_Document_ID	uniqueidentifier	= try_cast(ltrim(rtrim(@i_Register_to_Document_ID	))	as uniqueidentifier)

	declare @e int = 0, @Cnt int = 0, @TranCount int = @@TranCount, @CntParam nvarchar(max) = N'', @RowStatus nvarchar(10) = '200'
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_Register_to_Document_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		-- Not null check and Convert check	
		if (@i_Register_to_Document_ID	is null ) select Id=50101, Msg=formatmessage(50101, 'Register_to_Document_ID'	) else if (@u_Register_to_Document_ID	is null ) select Id=50108, Msg=formatmessage(50108, 'Register_to_Document_ID'	,try_cast(@i_Register_to_Document_ID	as nvarchar(36)));set @e=@e+@@rowcount
		...
		select Id=50100, Msg=formatmessage(50100, N'Register_to_Document.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	...
	if (@e = 0) begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'Register_to_Document.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	if (@TranCount = 0) and (@e = 0) -- do commit only in case we opened transaction inside THIS procedure
		commit
	if (@e <> 0) begin -- if any error appear  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_Register_to_Document_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
