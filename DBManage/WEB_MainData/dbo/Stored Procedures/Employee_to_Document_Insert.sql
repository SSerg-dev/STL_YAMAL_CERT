...
CREATE procedure [dbo].[Employee_to_Document_Insert]	-- not null ->	+
(	 @i_RowStatus				nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_Employee_to_Document_ID		uniqueidentifier	= newid()
	...
	begin try
		...
		select @CntParam = N'Employee_ID="'+try_cast(@u_Employee_ID as nvarchar(max))+N'",Document_ID="'+try_cast(@u_Document_ID as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Employee_to_Document_Insert.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action
	-------------
	-- Employee
	if (@e = 0) begin try
		Insert into dbo.p_Employee_to_Document
		(	 Employee_to_Document_ID
			,RowStatus
			,Insert_DTS
			,Update_DTS 
			,Created_User_ID
		...
		select Id=50100, Msg=formatmessage(50100, N'Employee_to_Document_Insert.Insert', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action
	-------------
	...
	if @e > 0 select @o_IsError = 1, @o_Entity_ID = null else select @o_IsError = 0, @o_Entity_ID = try_cast(@u_Employee_to_Document_ID as nvarchar(36))
