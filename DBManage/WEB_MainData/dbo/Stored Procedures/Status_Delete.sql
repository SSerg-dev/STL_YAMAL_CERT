...
CREATE procedure [dbo].[Status_Delete]	-- not null ->	+
(	 @i_AppUser_ID									nvarchar(250)		-- sys param	+
	...
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_Status_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'Status.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	-- Status
	if (@e = 0) begin try
		update dbo.p_Status set RowStatus = @RowStatus, Update_DTS = @u_Update_DTS,Modified_User_ID  = @u_Modified_User_ID where Status_ID = @u_Status_ID
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'Status.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_Status_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
