...
create procedure dbo.Document_to_GOST_Insert	-- not null ->	+
(	 @i_RowStatus				nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_Document_to_GOST_ID		uniqueidentifier	= newid()
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@TranCount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_Document_to_GOST_Insert
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select @CntParam = N'Document_ID="'+try_cast(@u_Document_ID as nvarchar(max))+N'",GOST_ID="'+try_cast(@u_GOST_ID as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Document_to_GOST_Insert.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action
	-------------
	-- Document
	if (@e = 0) begin try
		Insert into dbo.p_Document_to_GOST
		(	 Document_to_GOST_ID
			,RowStatus
			,Insert_DTS
			,Update_DTS 
		...
		select Id=50100, Msg=formatmessage(50100, N'Document_to_GOST_Insert.Insert', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_Document_to_GOST_Insert	-- rollback if transaction was created outside and this procedure creates only save point
	end
	-- Output parameters of the procedure 
	
	if @e > 0 select @o_IsError = 1, @o_Entity_ID = null else select @o_IsError = 0, @o_Entity_ID = try_cast(@u_Document_to_GOST_ID as nvarchar(36))
