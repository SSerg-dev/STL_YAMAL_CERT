...
create procedure [dbo].[CheckItem_To_Status_Delete]	-- not null ->	+
(	 @i_AppUser_ID				nvarchar(250)		-- sys param	+
	,@i_CheckItem_to_Status_ID	nvarchar(250)		-- usr param	+
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_CheckItem_to_Status_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		-- Not null check and Convert check	
		if (@i_CheckItem_to_Status_ID	is null ) select Id=50101, Msg=formatmessage(50101, 'CheckItem_to_Status_ID'	) else if (@u_CheckItem_to_Status_ID	is null ) select Id=50108, Msg=formatmessage(50108, 'CheckItem_to_Status_ID'	,try_cast(@i_CheckItem_to_Status_ID	as nvarchar(250)));set @e=@e+@@rowcount
		...
		select Id=50100, Msg=formatmessage(50100, N'CheckItem_to_Status.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	-- CheckItem
	if (@e = 0) begin try
		delete from dbo.p_CheckItem_To_Status where CheckItem_to_Status_ID = @u_CheckItem_to_Status_ID 
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'CheckItem_to_Status.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_CheckItem_to_Status_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
