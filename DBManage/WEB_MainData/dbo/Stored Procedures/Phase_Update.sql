...
create procedure dbo.Phase_Update	-- not null ->	+
(	 @i_Phase_ID		nvarchar(250)	-- sys param	+
	...
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_Phase_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		if (@i_Phase_ID is null ) select Id=50101, Msg=formatmessage(50101, 'Phase_ID' ) else if (@u_Phase_ID		is null ) select Id=50108, Msg=formatmessage(50108, 'Phase_ID' ,try_cast(@i_Phase_ID as nvarchar(36)));set @e=@e+@@rowcount
		...
		exec Get_RowCount 'p_Phase', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'Phase',try_cast(@u_Phase_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		
		select @CntParam = N'Phase_ID="'+try_cast(@u_Phase_ID as nvarchar(max))+N'",'+N'Phase_Code="'+try_cast(@u_Phase_Code as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Phase_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					from dbo.p_Phase
					where Phase_ID = @u_Phase_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
	...
	begin
		...
		if (@e = 0) begin try
			...
			where Phase_ID = @u_Phase_ID
		end try begin catch 
			select Id=50100, Msg=formatmessage(50100, N'Phase_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_Phase_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
