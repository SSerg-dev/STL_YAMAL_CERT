...
CREATE procedure [abd].[Document_Delete]	-- not null ->	+
(	 @i_Document_ID	nvarchar(250)		-- usr param	+
	...
	-- Document
	if (@e = 0) begin try
		update abd.p_Document set RowStatus = @RowStatus, Update_DTS = @u_Update_DTS, Modified_User_ID  = @u_Modified_User_ID where Document_ID = @u_Document_ID
	end try begin catch 
