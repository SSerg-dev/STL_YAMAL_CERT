...
create procedure abd.RelationType_Update	-- not null ->	+
(	 @i_RelationType_ID			nvarchar(250)	-- sys param	+
	,@i_RowStatus					nvarchar(250)	-- sys param	+
	,@i_AppUser_ID					nvarchar(250)	-- sys param	+
	,@i_RelationType_Code			nvarchar(250)	-- usr param	+
	...
	declare 
		...
		,@u_RelationType_Code	nvarchar(255)		= try_cast(ltrim(rtrim(@i_RelationType_Code))		as nvarchar(255))

		,@u_RelationType_ID			uniqueidentifier	= try_cast(ltrim(rtrim(@i_RelationType_ID	)) as uniqueidentifier)
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@TranCount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @TranCount = 0 begin transaction	else save transaction trn_RelationType_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		if (@i_RelationType_ID		is null)select Id=50101,Msg=formatmessage(50101,'RelationType_ID	')else if(@u_RelationType_ID	is null)select Id=50108,Msg=formatmessage(50108,'RelationType_ID	',try_cast(@i_RelationType_ID		as nvarchar(250)));set @e=@e+@@rowcount
		if (@i_RelationType_Code	is null)select Id=50101,Msg=formatmessage(50101,'RelationType_Code	')else if(@u_RelationType_Code	is null)select Id=50103,Msg=formatmessage(50103,'RelationType_Code	',try_cast(@i_RelationType_Code		as nvarchar(250)));set @e=@e+@@rowcount
		...
		select @CntParam = N'RelationType_ID="'+try_cast(@u_RelationType_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_RelationType', @CntParam, @Cnt out; if (@Cnt <=0) 
			select Id=50122,Msg=formatmessage(50122,'RelationType',try_cast(@u_RelationType_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		
		select @CntParam = N'RelationType_ID="'+try_cast(@u_RelationType_ID as nvarchar(max))+N'",'+N'RelationType_Code="'+try_cast(@u_RelationType_Code as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_RelationType', @CntParam, @Cnt out; if (@Cnt>0) 
			select Id=50123,Msg=formatmessage(50123, try_cast(@u_RelationType_Code as nvarchar(250)), 'RelationType'); set @e=@e+@@rowcount

	end try	begin catch	
		select Id=50100,Msg=formatmessage(50100, N'RelationType_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					where RelationType_ID = @u_RelationType_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
				(select 
					...
					,RelationType_Code			= @u_RelationType_Code
				) x				  
			) b	on a.x = b.x
		)
	begin
		-- RelationType
		if (@e = 0) begin try
			update abd.p_RelationType set
				 RowStatus				= @u_RowStatus	 
				,Update_DTS				= @u_Update_DTS	 
				,Modified_User_ID		= @u_Modified_User_ID
				,RelationType_Code		= @u_RelationType_Code
			where RelationType_ID = @u_RelationType_ID
		end try begin catch 
			select Id=50100,Msg=formatmessage(50100, N'RelationType_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @TranCount > 0 rollback transaction trn_RelationType_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
