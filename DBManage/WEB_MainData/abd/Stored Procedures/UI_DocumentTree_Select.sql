...
(	 @i_Employee_ID			nvarchar(250)		-- usr	param	+
	...
	declare 
		 ...
		 @x_ContAppendix_RelationType_ID uniqueidentifier = (select top 1 RelationType_ID 
															from abd.p_RelationType 
															where RelationType_Code = 'Container-Appendix')
	...
	If @u_Employee_ID is not null
		...
		select 
				...
				COALESCE(drt.DocumentFrom_ID, '00000000-0000-0000-0000-000000000000')	as ParentDoc_ID,
		...
		into #1	
			...
			left join abd.p_DocumentRelation drt on dp.Document_ID = drt.DocumentTo_ID and drt.RelationType_ID = @x_ParentChild_RelationType_ID 
			left join abd.p_DocumentRelation drf on dp.Document_ID = drf.DocumentTo_ID and drt.RelationType_ID = @x_ContAppendix_RelationType_ID 
			left join abd.p_Document t on t.Document_ID = drt.DocumentFrom_ID 
			where 1=2
		...
		as	(
			...
			join abd.p_Document dp on dt.DocumentType_ID = dp.DocumentType_ID and dp.RowStatus = 300
			left join abd.p_DocumentRelation drt on dp.Document_ID = drt.DocumentTo_ID and drt.RelationType_ID = @x_ParentChild_RelationType_ID 
			left join abd.p_DocumentRelation drf on dp.Document_ID = drf.DocumentTo_ID and drf.RelationType_ID = @x_ContAppendix_RelationType_ID 
			left join abd.p_Document t on t.Document_ID = drt.DocumentFrom_ID
			where drf.RelationType_ID is null or drf.RelationType_ID <> @x_ContAppendix_RelationType_ID
			union ALL
			SELECT null,null,
