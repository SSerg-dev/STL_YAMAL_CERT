...
-- =============================================
CREATE PROCEDURE [syslog].[UserTaskLog_Insert] 
...
SET @Reference_Error = 0
	...
	Select	@x_Count = count(UserTask_ID) from syslog.UserTask where UserTask_ID = @i_UserTask_ID 
	If	@x_Count = 0  SET @Reference_Error = @Reference_Error + 1  

	IF @i_UserTaskMessage_ID IS NOT NULL
	begin
		Select	@x_Count = count(UserTaskMessage_ID) from syslog.UserTaskMessage where UserTaskMessage_ID = @i_UserTaskMessage_ID 
		If	@x_Count = 0  SET @Reference_Error = @Reference_Error + 1  
	end 
	...
	IF @Reference_Error + @User_Error = 0 
		...
		SET @o_DB_New_ID = NEWID()
			...
			Insert into [syslog].[p_UserTaskLog]
