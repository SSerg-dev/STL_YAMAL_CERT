...
CREATE procedure [abd].[DocumentAttribute_Delete]	-- not null ->	+
(	 @i_DocumentAttribute_ID	nvarchar(250)		-- usr param	+
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_DocumentAttr_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		exec Get_RowCount 'abd.p_DocumentAttribute', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'DocumentAttribute',try_cast(@u_DocumentAttribute_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		--== place your params here =

	end try	begin catch	
		select Id=50100, Msg=formatmessage(50100, N'DocumentAttribute.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	-- DocumentAttribute
	if (@e = 0) begin try
		update abd.p_DocumentAttribute set RowStatus = @RowStatus, Update_DTS = @u_Update_DTS, Modified_User_ID = @u_Modified_User_ID where DocumentAttribute_ID = @u_DocumentAttribute_ID
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'DocumentAttribute.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_DocumentAttr_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
