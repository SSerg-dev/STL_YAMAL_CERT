...
create procedure abd.DocumentType_Update	-- not null ->	+
(	 @i_DocumentType_ID			nvarchar(250)	-- sys param	+
	...
	begin try
		...
		exec Get_RowCount 'p_DocumentType', @CntParam, @Cnt out; if (@Cnt <=0) 
			select Id=50122,Msg=formatmessage(50122,'DocumentType',try_cast(@u_DocumentType_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					,DocumentType_Name			= @u_DocumentType_Name			
					,DocumentType_Group		= @u_DocumentType_Group			
				) x				  
			) b	on a.x = b.x
		)
	begin
