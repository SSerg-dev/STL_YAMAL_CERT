...
CREATE procedure [dbo].[Parameter_Update]	-- not null ->	+
(	 @i_Parameter_ID	nvarchar(250)	-- sys param	+
	,@i_RowStatus								nvarchar(250)	-- sys param	+
	,@i_AppUser_ID								nvarchar(250)	-- sys param	+
	,@i_Parameter_Code	nvarchar(250)	-- usr param	+
	,@i_Parameter_Value		nvarchar(1000)
	...
	declare 
		...
		,@u_Parameter_Code	nvarchar(255)		= try_cast(ltrim(rtrim(@i_Parameter_Code))	as nvarchar(255))
		,@u_Parameter_Value	nvarchar(1000)		= try_cast(ltrim(rtrim(@i_Parameter_Value	))	as nvarchar(255))
		--== place your params here ==

		,@u_Parameter_ID	uniqueidentifier = try_cast(ltrim(rtrim(@i_Parameter_ID	)) as uniqueidentifier)
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@trancount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Parameter_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		if (@i_Parameter_ID is null ) select Id=50101, Msg=formatmessage(50101, 'Parameter_ID' ) else if (@u_Parameter_ID		is null ) select Id=50108, Msg=formatmessage(50108, 'Parameter_ID' ,try_cast(@i_Parameter_ID as nvarchar(36)));set @e=@e+@@rowcount
		if (@i_Parameter_Code is null ) select Id=50101, Msg=formatmessage(50101, 'Parameter_Code' ) else if (@u_Parameter_Code	is null ) select Id=50103, Msg=formatmessage(50103, 'Parameter_Code' ,try_cast(@i_Parameter_Code as nvarchar(250)));set @e=@e+@@rowcount
		--== place your params here =
		...
		select @CntParam = N'Parameter_ID="'+try_cast(@u_Parameter_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_Parameter', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'Parameter',try_cast(@u_Parameter_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		
		select @CntParam = N'Parameter_ID="'+try_cast(@u_Parameter_ID as nvarchar(max))+N'",'+N'Parameter_Code="'+try_cast(@u_Parameter_Code as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_Parameter', @CntParam, @Cnt out; if (@Cnt>0) select Id=50123, Msg=formatmessage(50123, try_cast(@u_Parameter_Code as nvarchar(250)), 'Parameter'); set @e=@e+@@rowcount
		--== place your params here =

	end try	begin catch	
		select Id=50100, Msg=formatmessage(50100, N'Parameter_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					,Parameter_Code
					,Parameter_Value
					from dbo.p_Parameter
					where Parameter_ID = @u_Parameter_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
				(select 
					 ...
					 ,Parameter_Code  = @u_Parameter_Code
					 ,Parameter_Value = @u_Parameter_Value
				) x				  
			) b	on a.x = b.x
		)
	begin
		...
		if (@e = 0) begin try
			...
			where Parameter_ID = @u_Parameter_ID
		end try begin catch 
			select Id=50100, Msg=formatmessage(50100, N'Parameter_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Parameter_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
