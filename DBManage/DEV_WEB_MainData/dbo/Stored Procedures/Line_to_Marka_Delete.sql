...
create procedure dbo.Line_to_Marka_Delete	-- not null ->	+
(	 @i_AppUser_ID			nvarchar(250)		-- sys param	+
	,@i_Line_to_Marka_ID	nvarchar(250)		-- usr param	+
	...
	declare 
		...
		,@u_Line_to_Marka_ID		uniqueidentifier	= try_cast(ltrim(rtrim(@i_Line_to_Marka_ID	))	as uniqueidentifier	)
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@trancount, @CntParam nvarchar(max) = N'', @RowStatus nvarchar(10) = '200'
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Line_to_Marka_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		-- Not null check and Convert check	
		if (@i_Line_to_Marka_ID	is null ) select Id=50101, Msg=formatmessage(50101, 'Line_to_Marka_ID'	) else if (@u_Line_to_Marka_ID	is null ) select Id=50108, Msg=formatmessage(50108, 'Line_to_Marka_ID'	,try_cast(@i_Line_to_Marka_ID	as nvarchar(36)));set @e=@e+@@rowcount
		...
		select Id=50100, Msg=formatmessage(50100, N'Line_to_Marka.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	-- Line
	if (@e = 0) begin try
		update p_Line_To_Marka set 
			RowStatus = 200, 
			Update_DTS = @u_Update_DTS, 
			Modified_User_ID  = @u_Modified_User_ID
			where Line_to_Marka_ID = @u_Line_to_Marka_ID
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'Line_to_Marka.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Line_to_Marka_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
