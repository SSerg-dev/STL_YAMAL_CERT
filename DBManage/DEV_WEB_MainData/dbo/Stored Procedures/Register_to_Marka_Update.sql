...
CREATE procedure [dbo].[Register_to_Marka_Update]		-- not null ->	+
(	 @i_Register_to_Marka_ID		nvarchar(250)	-- sys param	+
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Reg_to_Marka_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select @CntParam = N'Register_to_Marka_ID="'+try_cast(@u_Register_to_Marka_ID as nvarchar(max))+N'",'+N'Register_ID="'+try_cast(@u_Register_ID as nvarchar(max))+N'",'+N'Marka_ID="'+try_cast(@u_Marka_ID as nvarchar(max))+N'",Iteration="'+try_cast(@u_Iteration as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Register_to_Marka_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					,Marka_ID	
					,Iteration	
					from p_Register_to_Marka
					where Register_to_Marka_ID = @u_Register_to_Marka_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
	...
	begin
		...
		if (@e = 0) begin try
			...
			select Id=50100, Msg=formatmessage(50100, N'Register_to_Marka_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Reg_to_Marka_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
