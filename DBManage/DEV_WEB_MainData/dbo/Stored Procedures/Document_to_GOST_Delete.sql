...
CREATE procedure [dbo].[Document_to_GOST_Delete]	-- not null ->	+
(	 @i_AppUser_ID									nvarchar(250)		-- sys param	+
	,@i_Document_to_GOST_ID	nvarchar(250)		-- usr param	+
	...
	declare 
		...
		,@u_Document_to_GOST_ID	uniqueidentifier	= try_cast(ltrim(rtrim(@i_Document_to_GOST_ID	))	as uniqueidentifier	)
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@trancount, @CntParam nvarchar(max) = N'', @RowStatus nvarchar(10) = '200'
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Document_to_GOST_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		-- Not null check and Convert check	
		if (@i_Document_to_GOST_ID	is null ) select Id=50101, Msg=formatmessage(50101, 'Document_to_GOST_ID'	) else if (@u_Document_to_GOST_ID	is null ) select Id=50108, Msg=formatmessage(50108, 'Document_to_GOST_ID'	,try_cast(@i_Document_to_GOST_ID	as nvarchar(36)));set @e=@e+@@rowcount
		...
		select Id=50100, Msg=formatmessage(50100, N'Document_to_GOST.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	...
	if (@e = 0) begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'Document_to_GOST.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Document_to_GOST_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
