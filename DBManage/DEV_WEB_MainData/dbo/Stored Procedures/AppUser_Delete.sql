...
-- =============================================
create procedure AppUser_Delete	-- not null ->	+
(	 @i_AppUser_ID		nvarchar(250)	-- sys param	+
	,@i_AppUserField_ID	nvarchar(250)	-- usr param	+
	...
	declare 
		...
		,@u_AppUserField_ID	uniqueidentifier	= try_cast(ltrim(rtrim(@i_AppUser_ID	))	as uniqueidentifier	)
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@trancount, @CntParam nvarchar(max) = N'', @RowStatus int = 200
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_AppUser_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		-- Not null check and Convert check	
		if (@i_AppUser_ID	is null ) select Id=50101, Msg=formatmessage(50101, 'AppUser_ID'	) else if (@u_AppUserField_ID	is null ) select Id=50108, Msg=formatmessage(50108, 'AppUser_ID'	,try_cast(@i_AppUserField_ID	as nvarchar(36)));set @e=@e+@@rowcount
		...
		select @CntParam = N'AppUser_ID="'+try_cast(@u_AppUserField_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_AppUser', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'AppUser',try_cast(@u_AppUserField_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		--== place your params here =

	end try	begin catch	
		select Id=50100, Msg=formatmessage(50100, N'AppUser.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	-- AppUser
	if (@e = 0) begin try
		update dbo.p_AppUser set RowStatus = @RowStatus, Update_DTS = @u_Update_DTS,Modified_User_ID  = @u_Modified_User_ID where AppUser_ID = @u_AppUserField_ID
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'AppUser.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_AppUser_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
