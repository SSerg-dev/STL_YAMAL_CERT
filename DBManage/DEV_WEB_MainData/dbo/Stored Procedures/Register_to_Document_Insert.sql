...
CREATE procedure [dbo].[Register_to_Document_Insert]	-- not null ->	+
(	 @i_RowStatus				nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_Register_To_Document_ID		uniqueidentifier	= newid()
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@trancount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Register_to_Document_Insert
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		if (@i_Iteration		is null ) select Id=50101, Msg=formatmessage(50101, 'Iteration'			) else if (@u_Iteration			is null ) select Id=50102, Msg=formatmessage(50102, 'Iteration'				,try_cast(@i_Iteration				as nvarchar(250)));set @e=@e+@@rowcount
		if (@i_NumberInRegister		is not null ) and (@u_NumberInRegister	is null )															  select Id=50102, Msg=formatmessage(50102, 'NumberInRegister'		,try_cast(@i_NumberInRegister		as nvarchar(250)));set @e=@e+@@rowcount
		if (@i_SheetFolderNumber	is not null ) and (@u_SheetFolderNumber is null )															  select Id=50102, Msg=formatmessage(50102, 'SheetFolderNumber'		,try_cast(@i_SheetFolderNumber		as nvarchar(250)));set @e=@e+@@rowcount
		if (@i_Comment				is not null ) and (@u_Comment			is null )															  select Id=50103, Msg=formatmessage(50103, 'Comment'				,try_cast(@i_Comment				as nvarchar(250)));set @e=@e+@@rowcount
		...
		select @CntParam = N'Register_ID="'+try_cast(@u_Register_ID as nvarchar(max))+N'",Document_ID="'+try_cast(@u_Document_Id as nvarchar(max))+N'",Iteration="'+try_cast(@u_Iteration as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Register_to_Document_Insert.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action
	-------------
	-- Register
	if (@e = 0) begin try
		Insert into dbo.p_Register_to_Document
		(	 Register_to_Document_ID
			,RowStatus
			,Insert_DTS
			,Update_DTS 
			,Created_User_ID
		...
		select Id=50100, Msg=formatmessage(50100, N'Register_to_Document_Insert.Insert', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Register_to_Document_Insert	-- rollback if transaction was created outside and this procedure creates only save point
	end
	-- Output parameters of the procedure 
	
	if @e > 0 select @o_IsError = 1, @o_Entity_ID = null else select @o_IsError = 0, @o_Entity_ID = try_cast(@u_Register_To_Document_ID as nvarchar(36))
