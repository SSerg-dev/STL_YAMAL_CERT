...
CREATE procedure [dbo].[Register_to_Status_Update]	-- not null ->	+
(	 @i_Register_to_Status_ID	nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_Register_To_Status_ID		uniqueidentifier	= try_cast(ltrim(rtrim(@i_Register_to_Status_ID	))	as uniqueidentifier	)
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Register_to_Status_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select @CntParam = N'Register_to_Status_ID="'+try_cast(@u_Register_to_Status_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_Register_to_Status', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'Register_to_Status',try_cast(@u_Register_to_Status_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		
		select @CntParam = N'Register_to_Status_ID="'+try_cast(@u_Register_to_Status_ID as nvarchar(max))+N'",'+N'Register_ID="'+try_cast(@u_Register_ID as nvarchar(max))+N'",'+N'Status_ID="'+try_cast(@u_Status_ID as nvarchar(max))+N'",Iteration="'+try_cast(@u_Iteration as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Register_to_Status_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					,DTS_Start
					,DTS_End
					from p_Register_to_Status
					where Register_to_Status_ID = @u_Register_to_Status_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
	...
	begin
		-- Register
		if (@e = 0) begin try
			update dbo.p_Register_to_Status set
				 RowStatus			= @u_RowStatus	 
				...
				,DTS_Start			= @u_DTS_Start
				,DTS_End			= @u_DTS_End 
			where Register_to_Status_ID = @u_Register_To_Status_ID
		end try begin catch 
			select Id=50100, Msg=formatmessage(50100, N'Register_to_Status_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Register_to_Status_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
