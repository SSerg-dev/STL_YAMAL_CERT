...
create procedure dbo.Employee_to_GeneralDocument_Update	-- not null ->	+
(	 @i_Employee_to_GeneralDocument_ID	nvarchar(250)	-- sys param	+
	,@i_RowStatus				nvarchar(250)	-- sys param	+
	,@i_AppUser_ID									nvarchar(250)	-- sys param	+
	,@i_Employee_ID				nvarchar(250)	-- usr param	+
	,@i_GeneralDocument_ID				nvarchar(250)	-- usr param	+
	...
	declare 
		...
		,@u_GeneralDocument_ID					uniqueidentifier	= try_cast(ltrim(rtrim(@i_GeneralDocument_ID			))	as uniqueidentifier)
		,@u_Employee_to_GeneralDocument_ID		uniqueidentifier	= try_cast(ltrim(rtrim(@i_Employee_to_GeneralDocument_ID	))	as uniqueidentifier)
	...
	begin try
		...
		if (@i_GeneralDocument_ID		is null ) select Id=50101, Msg=formatmessage(50101, 'GeneralDocument_ID'		) else if (@u_GeneralDocument_ID	is null ) select Id=50108, Msg=formatmessage(50108, 'GeneralDocument_ID'	,try_cast(@i_GeneralDocument_ID	as nvarchar(250)));set @e=@e+@@rowcount
		...
		select @CntParam = N'GeneralDocument_ID="'+try_cast(@u_GeneralDocument_ID as nvarchar(max))+'"', @Cnt=-1
		exec Get_RowCount 'p_GeneralDocument', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'p_GeneralDocument', try_cast(@u_GeneralDocument_ID as NVARCHAR(36))); set @e=@e+@@rowcount

		select @CntParam = N'Employee_to_GeneralDocument_ID="'+try_cast(@u_Employee_to_GeneralDocument_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_Employee_to_GeneralDocument', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'Employee_to_GeneralDocument',try_cast(@u_Employee_to_GeneralDocument_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		
		select @CntParam = N'Employee_to_GeneralDocument_ID="'+try_cast(@u_Employee_to_GeneralDocument_ID as nvarchar(max))+N'",'+N'Employee_ID="'+try_cast(@u_Employee_ID as nvarchar(max))+N'",'+N'GeneralDocument_ID="'+try_cast(@u_GeneralDocument_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_Employee_to_GeneralDocument', @CntParam, @Cnt out; if (@Cnt>0) select Id=50123, Msg=formatmessage(50123, try_cast(@u_Employee_to_GeneralDocument_ID as nvarchar(36)), 'Employee_to_GeneralDocument'); set @e=@e+@@rowcount

		--== place your params here =

	end try	begin catch	
		select Id=50100, Msg=formatmessage(50100, N'Employee_to_GeneralDocument_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					,Employee_ID
					,GeneralDocument_ID	
					from dbo.p_Employee_to_GeneralDocument
					where Employee_to_GeneralDocument_ID = @u_Employee_to_GeneralDocument_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
				(select 
					 ...
					 ,GeneralDocument_ID	   = @u_GeneralDocument_ID	
				) x				  
			) b	on a.x = b.x
		)
	begin
		-- Employee
		if (@e = 0) begin try
			update dbo.p_Employee_to_GeneralDocument set
				 RowStatus	  = @u_RowStatus	 
				,Update_DTS	  = @u_Update_DTS	 
				,Modified_User_ID  = @u_Modified_User_ID
				,Employee_Id  = @u_Employee_ID 
				,GeneralDocument_ID	  = @u_GeneralDocument_ID	 
			where Employee_to_GeneralDocument_ID = @u_Employee_To_GeneralDocument_ID
		end try begin catch 
			select Id=50100, Msg=formatmessage(50100, N'Employee_to_GeneralDocument_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
