...
CREATE procedure [dbo].[Position_Delete]	-- not null ->	+
(	 @i_Position_ID	nvarchar(250)		-- usr param	+
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Position_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		if (@i_AppUser_ID		is null ) select Id=50101, Msg=formatmessage(50101, 'User_ID'		) else if (@u_Modified_User_ID	is null ) select Id=50108, Msg=formatmessage(50108, 'User_ID'		,try_cast(@i_AppUser_ID		as nvarchar));set @e=@e+@@rowcount
		...
		exec Get_RowCount 'p_RowStatus', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'p_RowStatus', try_cast(@RowStatus as NVARCHAR(50))); set @e=@e+@@rowcount
		...
		select Id=50100, Msg=formatmessage(50100, N'Position.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	-- Position
	if (@e = 0) begin try
		update dbo.p_Position set RowStatus = @RowStatus, Update_DTS = @u_Update_DTS,Modified_User_ID  = @u_Modified_User_ID where Position_ID = @u_Position_ID
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'Position.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Position_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
