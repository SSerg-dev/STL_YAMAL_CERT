...
create procedure dbo.WorkPackage_Delete	-- not null ->	+
(	 @i_WorkPackage_ID	nvarchar(250)		-- usr param	+
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_WorkPackage_Delete
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'WorkPackage.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
	-- WorkPackage
	if (@e = 0) begin try
		update dbo.p_WorkPackage set RowStatus = @RowStatus, Update_DTS = @u_Update_DTS,Modified_User_ID  = @u_Modified_User_ID where WorkPackage_ID = @u_WorkPackage_ID
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'WorkPackage.Delete', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_WorkPackage_Delete	-- rollback if transaction was created outside and this procedure creates only save point
	end
