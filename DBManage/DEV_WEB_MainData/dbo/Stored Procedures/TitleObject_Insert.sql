...
CREATE procedure [dbo].[TitleObject_Insert]	-- not null ->	+
(	 @i_RowStatus			nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_TitleObject_ID			uniqueidentifier	= newid()
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@trancount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_TitleObject_Insert
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select @CntParam = N'TitleObject_Code="'+try_cast(@u_TitleObject_Code as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'TitleObject_Insert.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------		
	...
	if (@e = 0) begin try
		...
		(	 @u_TitleObject_ID
			...
			,@u_Book3_Weight						
			,@u_Book4_Weight						
			,@u_TitleObject_for_ABDFinalSet			
			,@u_Book1_Documents_Total				
			,@u_Book1_Documents_received			
			,@u_Book1_Progress						
			,@u_Book1_Documents_transmitted_to_CPY	
			,@u_StageOfConstr	
		)
	end try begin catch 
		select Id=50100, Msg=formatmessage(50100, N'TitleObject_Insert.Insert', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action 
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_TitleObject_Insert	-- rollback if transaction was created outside and this procedure creates only save point
	end
	-- Output parameters of the procedure 
	
	if @e > 0 select @o_IsError = 1, @o_Entity_ID = null else select @o_IsError = 0, @o_Entity_ID = try_cast(@u_TitleObject_ID as nvarchar(36))
