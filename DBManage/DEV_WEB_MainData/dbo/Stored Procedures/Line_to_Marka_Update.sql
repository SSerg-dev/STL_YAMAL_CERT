...
create procedure dbo.Line_to_Marka_Update	-- not null ->	+
(	 @i_Line_to_Marka_ID		nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_Line_To_Marka_ID			uniqueidentifier	= try_cast(ltrim(rtrim(@i_Line_to_Marka_ID	))	as nvarchar(255))
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@trancount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Line_to_Marka_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select @CntParam = N'Line_to_Marka_ID="'+try_cast(@u_Line_to_Marka_ID as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_Line_to_Marka', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'Line_to_Marka',try_cast(@u_Line_to_Marka_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		
		select @CntParam = N'Line_to_Marka_ID="'+try_cast(@u_Line_to_Marka_ID as nvarchar(max))+N'",'+N'Line_ID="'+try_cast(@u_Line_ID as nvarchar(max))+N'",'+N'Marka_ID="'+try_cast(@u_Marka_ID as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Line_to_Marka_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					,Line_Id
					,Marka_ID	
					from p_Line_to_Marka
					where Line_to_Marka_ID = @u_Line_to_Marka_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
	...
	begin
		-- Line
		if (@e = 0) begin try
			update dbo.p_Line_to_Marka set
				 RowStatus	  = @u_RowStatus	 
				,Update_DTS	  = @u_Update_DTS	 
				,Modified_User_ID  = @u_Modified_User_ID
				,Line_Id  = @u_Line_Id 
				,Marka_ID	  = @u_Marka_ID	 
			where Line_to_Marka_ID = @u_Line_To_Marka_ID
		end try begin catch 
			select Id=50100, Msg=formatmessage(50100, N'Line_to_Marka_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Line_to_Marka_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
