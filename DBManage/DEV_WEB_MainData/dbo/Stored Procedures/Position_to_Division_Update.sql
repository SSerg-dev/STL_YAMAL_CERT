...
create procedure dbo.Position_to_Division_Update	-- not null ->	+
(	 @i_Position_to_Division_ID	nvarchar(250)	-- sys param	+
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Position_to_Division_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		select @CntParam = N'Position_to_Division_ID="'+try_cast(@u_Position_to_Division_ID as nvarchar(max))+N'",'+N'Position_ID="'+try_cast(@u_Position_ID as nvarchar(max))+N'",'+N'Division_ID="'+try_cast(@u_Division_ID as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Position_to_Division_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					where Position_to_Division_ID = @u_Position_to_Division_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
	...
	begin
		...
		if (@e = 0) begin try
			...
			select Id=50100, Msg=formatmessage(50100, N'Position_to_Division_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Position_to_Division_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
