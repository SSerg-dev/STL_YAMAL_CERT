...
create procedure dbo.ProcessPhase_Update	-- not null ->	+
(	 @i_ProcessPhase_ID		nvarchar(250)	-- sys param	+
	,@i_RowStatus		nvarchar(250)	-- sys param	+
	,@i_AppUser_ID		nvarchar(250)	-- sys param	+
	,@i_ProcessPhase_Code		nvarchar(250)	-- usr param	+
	...
	declare 
		...
		,@u_ProcessPhase_Code						nvarchar(255)		= try_cast(ltrim(rtrim(@i_ProcessPhase_Code))	as nvarchar(255))
		--== place your params here ==
	...
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_ProcessPhase_Update
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		if (@i_ProcessPhase_ID is null ) select Id=50101, Msg=formatmessage(50101, 'ProcessPhase_ID' ) else if (@u_ProcessPhase_ID		is null ) select Id=50108, Msg=formatmessage(50108, 'ProcessPhase_ID' ,try_cast(@i_ProcessPhase_ID as nvarchar(36)));set @e=@e+@@rowcount
		if (@i_ProcessPhase_Code is null ) select Id=50101, Msg=formatmessage(50101, 'ProcessPhase_Code' ) else if (@u_ProcessPhase_Code	is null ) select Id=50103, Msg=formatmessage(50103, 'ProcessPhase_Code' ,try_cast(@i_ProcessPhase_Code as nvarchar(250)));set @e=@e+@@rowcount
		--== place your params here =
		...
		exec Get_RowCount 'p_ProcessPhase', @CntParam, @Cnt out; if (@Cnt<=0) select Id=50122, Msg=formatmessage(50122,'ProcessPhase',try_cast(@u_ProcessPhase_ID as NVARCHAR(36))); set @e=@e+@@rowcount
		
		select @CntParam = N'ProcessPhase_ID="'+try_cast(@u_ProcessPhase_ID as nvarchar(max))+N'",'+N'ProcessPhase_Code="'+try_cast(@u_ProcessPhase_Code as nvarchar(max))+N'"', @Cnt=-1
		exec Get_RowCount 'p_ProcessPhase', @CntParam, @Cnt out; if (@Cnt>0) select Id=50123, Msg=formatmessage(50123, try_cast(@u_ProcessPhase_Code as nvarchar(250)), 'ProcessPhase'); set @e=@e+@@rowcount
		--== place your params here =

	end try	begin catch	
		select Id=50100, Msg=formatmessage(50100, N'ProcessPhase_Update.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action 
	-------------
		...
		(-- check if any changes appear
			...
			(select binary_checksum(*) x from
				(select 
					...
					from dbo.p_ProcessPhase
					where ProcessPhase_ID = @u_ProcessPhase_ID and RowStatus < 100
				) x
			) a
			join
			(select binary_checksum(*) x from
				(select 
					 ...
					 ,ProcessPhase_Code  = @u_ProcessPhase_Code
					 --== place your params here =
	...
	begin
		...
		if (@e = 0) begin try
			...
			where ProcessPhase_ID = @u_ProcessPhase_ID
		end try begin catch 
			select Id=50100, Msg=formatmessage(50100, N'ProcessPhase_Update.Update', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
		end catch
	end
	-------------
	------------- Final Action 
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_ProcessPhase_Update	-- rollback if transaction was created outside and this procedure creates only save point
	end
