...
CREATE procedure [dbo].[Document_to_Status_Insert]	-- not null ->	+
(	 @i_RowStatus				nvarchar(250)	-- sys param	+
	...
	declare 
		...
		,@u_Parent_ID					uniqueidentifier	= try_cast(ltrim(rtrim(@i_Parent_ID			))	as nvarchar(255))
		,@u_DTS_Start					datetime2(7)		= try_cast(getdate()							as datetime2(7))

		,@u_Document_To_Status_ID		uniqueidentifier	= newid()
	declare @e int = 0, @Cnt int = 0, @TranCount int = @@trancount, @CntParam nvarchar(max) = N''
	-- Nested Tran
	if @trancount = 0 begin transaction	else save transaction trn_Document_to_Status_Insert
	-------------
	------------- Error Handling 
	-------------
	begin try
		...
		if (@i_Parent_ID		is not null ) and (@u_Parent_ID is null )																	  select Id=50108, Msg=formatmessage(50108, 'Parent_ID'		,try_cast(@i_Status_ID		as nvarchar(36)));set @e=@e+@@rowcount
		...
		select @CntParam = N'Document_ID="'+try_cast(@u_Document_ID as nvarchar(max))+N'",Status_ID="'+try_cast(@u_Status_Id as nvarchar(max))+N'"', @Cnt=-1
		...
		select Id=50100, Msg=formatmessage(50100, N'Document_to_Status_Insert.Check', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Main Action
	-------------
	...
	if (@e = 0) begin try
		...
		select Id=50100, Msg=formatmessage(50100, N'Document_to_Status_Insert.Insert', error_message()); set @e=@e+@@rowcount; -- Unhandled Eror
	end catch
	-------------
	------------- Final Action
	-------------
	...
	if (@e <> 0) begin -- if any error apper  
		...
		if @xstate = 1 and @trancount > 0 rollback transaction trn_Document_to_Status_Insert	-- rollback if transaction was created outside and this procedure creates only save point
	end
	-- Output parameters of the procedure 
	
	if @e > 0 select @o_IsError = 1, @o_Entity_ID = null else select @o_IsError = 0, @o_Entity_ID = try_cast(@u_Document_To_Status_ID as nvarchar(36))
